...
-- ============================================================
-- Test 42: partitioned tables and table inheritance are rejected
-- ============================================================
-- Not supported: a pg_dump/pg_restore of an audited partition tree can lose
-- rows (pg_restore -j truncates a partition before loading it, and the
-- partition's triggers fire), and one of an audited inheritance child can
-- leave it unaudited (the restored child lists its inherited columns first,
-- out of line with its audit table). enable() refuses both up front and
-- creates nothing.
...
DECLARE
    t text;
BEGIN
    FOREACH t IN ARRAY ARRAY['public.test_proot', 'public.test_p1',
                             'public.test_child', 'public.test_parent'] LOOP
        BEGIN
            PERFORM pgaudix.enable(t::regclass);
            RAISE NOTICE 'ERROR: enable(%) should have been refused', t;
...
FROM pgaudix.monitored_tables
WHERE source_table IN ('test_proot', 'test_p1', 'test_child', 'test_parent');
SELECT count(*) AS audit_tables
FROM pg_catalog.pg_class
WHERE relname IN ('test_proot_audit', 'test_p1_audit', 'test_child_audit', 'test_parent_audit');
SELECT count(*) AS pgaudix_triggers
FROM pg_catalog.pg_trigger
WHERE tgname LIKE 'pgaudix\_%'
  AND tgrelid IN ('public.test_proot'::regclass, 'public.test_p1'::regclass,
                  'public.test_child'::regclass, 'public.test_parent'::regclass);

-- Once out of the hierarchy, both tables can be audited
ALTER TABLE public.test_child NO INHERIT public.test_parent;
SELECT pgaudix.enable('public.test_child');
SELECT pgaudix.enable('public.test_parent');
SELECT source_table, audit_table_exists, dml_trigger_enabled
FROM pgaudix.status()
WHERE source_table IN ('test_child', 'test_parent')
ORDER BY source_table;

SELECT pgaudix.disable('public.test_child', drop_data := true);
SELECT pgaudix.disable('public.test_parent', drop_data := true);
DROP TABLE public.test_child, public.test_parent, public.test_proot;

-- ============================================================
-- Test 43: an audited table cannot join a partition tree or an inheritance hierarchy
-- ============================================================
-- enable() refuses these tables, so DDL that would make an audited table a
-- partition (ATTACH PARTITION), an inheritance child (INHERIT) or an
-- inheritance parent (INHERIT from another table, CREATE TABLE ... INHERITS)
-- is refused by ddl_sync(), and the table stays audited as it was.
CREATE TABLE public.test_solo (id int, k int);
SELECT pgaudix.enable('public.test_solo');
CREATE TABLE public.test_other (id int, k int);
CREATE TABLE public.test_root (id int, k int) PARTITION BY LIST (k);
...
BEGIN
    FOREACH cmd IN ARRAY ARRAY[
        'ALTER TABLE public.test_root ATTACH PARTITION public.test_solo FOR VALUES IN (1)',
        'ALTER TABLE public.test_solo INHERIT public.test_other',
        'ALTER TABLE public.test_other INHERIT public.test_solo',
        'CREATE TABLE public.test_sub () INHERITS (public.test_solo)'] LOOP
        BEGIN
            EXECUTE cmd;
            RAISE NOTICE 'ERROR: should have been refused: %', cmd;
        EXCEPTION WHEN feature_not_supported THEN
            RAISE NOTICE 'OK: %', SQLERRM;
        END;
    END LOOP;
...
-- Nothing changed: no inheritance link, no new table, still audited
SELECT count(*) AS inheritance_links
FROM pg_catalog.pg_inherits
WHERE 'public.test_solo'::regclass IN (inhrelid, inhparent);
SELECT to_regclass('public.test_sub') IS NULL AS sub_not_created;
INSERT INTO public.test_solo VALUES (1, 1);
SELECT audit_operation, id, k FROM public.test_solo_audit ORDER BY audit_id;

-- Tables that are not audited use inheritance and partitioning freely
CREATE TABLE public.test_sub () INHERITS (public.test_other);
CREATE TABLE public.test_p2 PARTITION OF public.test_root FOR VALUES IN (2);
SELECT inhrelid::regclass::text AS child, inhparent::regclass::text AS parent
FROM pg_catalog.pg_inherits
WHERE inhparent IN ('public.test_other'::regclass, 'public.test_root'::regclass)
ORDER BY 1;

-- After disable() the table is free to join
SELECT pgaudix.disable('public.test_solo', drop_data := true);
ALTER TABLE public.test_root ATTACH PARTITION public.test_solo FOR VALUES IN (1);
SELECT count(*) AS partitions FROM pg_catalog.pg_inherits
WHERE inhparent = 'public.test_root'::regclass;

DROP TABLE public.test_root, public.test_sub, public.test_other;
...
-- ============================================================
-- A second TRUNCATE of the same table issued from a DO block or PL/pgSQL
-- function (same statement_timestamp()) is a separate operation.
